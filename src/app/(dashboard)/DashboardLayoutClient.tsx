...
/**
 * Inner dashboard layout. This file is imported via `next/dynamic({ ssr: false })`
 * from layout.tsx, which means it ONLY renders on the client. That lets us read
 * the persisted Zustand auth state synchronously on the first render — no
 * hydration flag, no spinner flash on reload.
 */
export default function DashboardLayoutClient({ children }: { children: React.ReactNode }) {
  ...
  // Open contributor modal from URL flag
  useEffect(() => {
    const shouldOpenModal = searchParams.get('openContributorModal') === 'true'

    if (shouldOpenModal && user && !(user.role === 'contributor' && user.isContributor)) {
  ...
  // Redirect to /login if logged out. Because this whole tree is ssr:false,
  // Zustand has already read localStorage synchronously, so this fires only
  ...
  // Logged-out users get redirected by the effect above; render nothing in the
  // meantime (no spinner, no scary "Restricted Access" message).
  if (!isLoggedIn || !user) {
    return null
  }
